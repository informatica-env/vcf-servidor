...
Version: 7.0 — caché, timeouts cortos y sin HTTP interno para SportsPress
...
function api_player_final($data) {
    ...
    if (!$html) {
        ...
        // seguidos 500 veces.
        // Antes de rendirnos: si hay una respuesta buena guardada, se sirve.
        $ultimo_bueno = get_transient($cache_key . '_ok');
        if ($ultimo_bueno !== false) {
            return $ultimo_bueno;
        }

        // Y si no la hay, que el error NO lo cachee Varnish. El 19/08/2026 se
        // vio uno servido con age=633: diez minutos de fichas sin datos por un
        // fallo de cuatro segundos.
        nocache_headers();
        header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0', true);

        $fail = ['error' => 'fetch failed', 'url' => $player_url];
        set_transient($cache_key, $fail, 10);
        return $fail;
    }
    ...
    // Copia de larga duracion. Si una peticion futura falla, se sirve esto en
    // vez de un error: mas vale un dato de hace un rato que una ficha vacia.
    set_transient($cache_key . '_ok', $out, WEEK_IN_SECONDS);
...
function parse_player($full, $body, $slug) {
    ...
    // La ficha puede tener hasta dos <ul> dentro de "biografia-jugador":
    // el primero son los equipos anteriores, y si existe un segundo, es el
    // palmarés. No todos los jugadores tienen palmarés, así que ese segundo
    // <ul> es opcional.
    ...
    // ===== BIO =====
    // Extraemos cada <p> del admin como su propio párrafo, en vez de
    // aplanar todo el texto y volver a "adivinar" los cortes de párrafo.
    ...
    // Por si queda algún encabezado suelto ("Equipos anteriores", "Palmarés"...)
    // como último párrafo tras quitar los <ul>, lo cortamos también.
